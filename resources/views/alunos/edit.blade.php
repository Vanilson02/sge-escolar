          ...
          <form method="post" action="{{ route('Aluno.update', $aluno) }}" autocomplete="off" class="form-horizontal">
            ...
            <div class="box ">
              <div class="box-header box-header-primary">
                <h4 class="box-title">{{ __('Editar Usuarios') }}</h4>
              ...
              <div class="box-body ">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-2">
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="nmatricula" id="input-name" type="number" readonly="readonly" placeholder="{{ __('Matricula') }}" value="{{ old('nmatricula', $aluno->NMATRICULA) }}" required="true" aria-required="true"/>
                      @if ($errors->has('name'))
                        <span id="name-error" class="error text-danger" for="input-name">{{ $errors->first('name') }}</span>
                      @endif
                    </div>
